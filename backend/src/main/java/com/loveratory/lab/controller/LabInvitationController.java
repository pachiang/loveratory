 ...
 * 實驗室邀請管理 API。
 * 提供邀請的建立、查詢、撤銷與接受功能。
 * 涵蓋 /api/v1/labs/{labId}/invitations 與 /api/v1/invitations/{token} 兩組路徑。
 */
@Slf4j
...
public class LabInvitationController {
    ...
    @GetMapping("/labs/{labId}/invitations")
    ...
    @PostMapping("/labs/{labId}/invitations")
    ...
    @DeleteMapping("/labs/{labId}/invitations/{invitationId}")
    ...
    public LabInvitationResponse acceptInvitation(
            @Parameter(description = "邀請 Token") @PathVariable String token) {
        log.info("接受實驗室邀請，token: {}", token);
        return labInvitationUseCase.acceptInvitationForCurrentUser(token);
    }

    @Operation(summary = "Register and accept invitation", description = "Register the invited account and accept the invitation in one step")
    @PostMapping("/invitations/{token}/register")
    public InvitationAccessResponse registerAndAcceptInvitation(
            @Parameter(description = "?隢?Token") @PathVariable String token,
            @Valid @RequestBody UserRegisterRequest request) {
        log.info("Register and accept invitation, token: {}, email: {}", token, request.getEmail());
        return labInvitationUseCase.registerAndAcceptInvitation(token, request);
    }

    @Operation(summary = "Login and accept invitation", description = "Login with the invited account and accept the invitation in one step")
    @PostMapping("/invitations/{token}/login")
    public InvitationAccessResponse loginAndAcceptInvitation(
            @Parameter(description = "?隢?Token") @PathVariable String token,
            @Valid @RequestBody UserLoginRequest request) {
        log.info("Login and accept invitation, token: {}, email: {}", token, request.getEmail());
        return labInvitationUseCase.loginAndAcceptInvitation(token, request);
    }
}
